...
export default function CategoryManagerModal({ team, onClose }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      const existing = team.customCategories || [];
      ...
      await sportsAPI.updateTeam(currentUser.uid, team.id, {
  ...
  const handleRemove = async (categoryId) => {
    ...
    await sportsAPI.updateTeam(currentUser.uid, team.id, {
      customCategories: (team.customCategories || []).filter(
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
        <div className="mb-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Custom to this team. Default categories stay locked.
            </p>
          </div>
        ...
        <div className="mb-5 divide-y divide-gray-200 rounded-xl border border-gray-200">
          {(team.customCategories || []).length === 0 ? (
            <p className="p-4 text-sm text-gray-500">No custom categories yet.</p>
          ) : (
            team.customCategories.map((category) => {
              const color = getCategoryColor(category);
